...
export function AssignProviderStaffDialog({
  user,
  open,
  onOpenChange,
  onAssigned,
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {results.map((provider) => (
                      ...
                      >
                        <Store size={14} className="mr-2 text-slate-500" />
                        {provider.name}
                        {selected === provider && (
...
export default function UserDetailPage() {
  ...
  const result = {
    ...
    data: {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      email: "sdicviuw@gmail.com",
      emailVerified: true,
      id: params.id || "saidwiemv-aszvwev-591dsv",
      name: "Tarek fawzy",
      role: "user",
      banExpires: null,
      banReason: null,
  ...
  return (
    <div className="px-4 md:px-6 py-6 space-y-6 p-6">
      ...
      <UserProviderReviewCard user={user} />
    </div>
  );
}

export function UserProviderReviewCard({ user }: { user: User }) {
    ...
    <SectionShell>
      ...
      <div className="mt-4 grid gap-3 grid-cols-2">
        <ActionCard
          ...
          description="Add this user directly to an existing provider's team."
          action={
            <Button
              variant="outline"
              className="w-full"
        ...
        <ActionCard
          ...
          description="Send a notification pointing them to the become-a-provider flow."
          action={
            <Button
              variant="outline"
              className="w-full"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="capitalize">
              Invite {user.name} to become a provider?
            </AlertDialogTitle>
            <AlertDialogDescription>
...
function SectionShell({ children }: { children: React.ReactNode }) {
  return <div className="rounded-md border p-5">{children}</div>;
}
